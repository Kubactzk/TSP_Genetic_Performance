...
namespace TSP_Genetic.Algorithms
{
    internal class GeneticParallel
    {
        ...
        public GeneticParallel((int, int)[] ints, (long, long)[] longs, (short, short)[] shorts, (double, double)[] doubles)
        {
            this.ints = doubles;
            this.longs = longs;
            this.shorts = shorts;
        ...
        public List<GeneticModel[]> Solve(int population_size, double mutation_rate, int generations)
        {
            GeneticModel[] population = new GeneticModel[population_size];
            var result = new ConcurrentBag<GeneticModel[]>();

            // Create a ParallelOptions instance and set MaxDegreeOfParallelism to 2
            ...
            // Add initial population to result (outside of parallel loop)
            result.Add(population);

            // Perform genetic algorithm iterations
            Parallel.For(0, generations - 1, parallelOptions, i =>
            {
                // Lock the modification of the population and result list
        ...
        public GeneticModel Generate_random_population()
        {
            ...
            model.fitness = weights.Sum(); // Assuming fitness is the sum of distances.
